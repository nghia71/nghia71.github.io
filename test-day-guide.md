---
title: How Test Day Works
---

# How test day works
# Ngày thi diễn ra như thế nào

One link, bookmarked once, carries a team through the whole year: opening a test paper, and submitting the solution. This page explains exactly what happens at each step, what the countdown means, and what to do if something goes sideways. It doesn't replace [Tests & Yearbook](./tests.md) — that page covers the test format and the yearbook; this one is the mechanics of an actual test day.

*Một đường link duy nhất, lưu lại một lần, dùng cho cả năm học: mở đề thi, và nộp bài giải. Trang này giải thích chính xác từng bước diễn ra như thế nào, đồng hồ đếm giờ có nghĩa là gì, và cần làm gì nếu có sự cố. Trang này không thay thế [Tests & Yearbook](./tests.md) — trang đó nói về cấu trúc đề thi và quyển lưu bút; trang này nói về cách vận hành thực tế của một ngày thi. Bản tiếng Việt nằm ngay dưới mỗi phần tiếng Anh, trong khung có tiêu đề* ***🇻🇳 Tiếng Việt.***

**Jump to:** [Glossary](#glossary) · [Parents & Students](#for-parents--students) · [Regional Coordinators](#for-regional-coordinators) · [Graders](#for-graders)
...
| Term | Meaning |
|---|---|
| **MCC Roster** (Public Roster) | The public spreadsheet listing every registered student and team — name, team, level, region, status. This is the page linked from [Organization](./organization.md); no password needed. |
...
| Comments | A grader's note, shown as a small note icon directly on that row's `Score` cell — click or hover it to read. Not a column of its own text. |
...
| Thuật ngữ | Ý nghĩa |
|---|---|
| **MCC Roster** (Public Roster) | Bảng tính công khai liệt kê mọi học sinh và đội đã đăng ký — tên, đội, level, khu vực, trạng thái. Đây là trang có link ở [Organization](./organization.md); không cần mật khẩu. |
...
| Comments (nhận xét) | Ghi chú của người chấm bài, hiện dưới dạng biểu tượng ghi chú nhỏ ngay trên ô `Score` của dòng đó — bấm hoặc rê chuột vào để đọc. Không phải một cột chữ riêng. |
...
### Checking your team on the Public Roster

Every registered student appears on the public **MCC Roster** — name, team, level, region, and status (`active`, `pending-team`, or `withdrawn`). No password needed; anyone can look. It's the fastest way to confirm your team, level, and region are entered correctly — worth a glance early in the year, or any time a level or team changes.

The roster deliberately shows only what a family needs to check, nothing more: it never includes age, parent contact details, or anything else — that stays private, on a separate internal sheet only Nghia and the coordinators can see.

If anything on your row looks wrong — a misspelled name, the wrong team, level, or region — tell your regional coordinator. They're the one who can fix it; it isn't something you can edit yourself.
...
The team's current state — for every test, every team — lives on the public **Exams & Standing** sheet (link on the [Organization](./organization.md) page). You don't need a password to view it; anyone can check it any time. Look for your team's row and its `Status` column, which is always in plain language and color-coded:
...
### Kiểm tra thông tin đội trên bảng Roster công khai

Mỗi học sinh đã đăng ký đều có tên trên bảng **MCC Roster** công khai — họ tên, đội, level, khu vực, và trạng thái (`active`, `pending-team`, hoặc `withdrawn`). Không cần mật khẩu; ai cũng xem được. Đây là cách nhanh nhất để kiểm tra đội, level, và khu vực của mình đã được nhập đúng chưa — nên xem qua đầu năm học, hoặc bất cứ khi nào level hay đội thay đổi.

Bảng Roster chỉ hiển thị đúng những gì một gia đình cần kiểm tra, không hơn: không bao giờ có tuổi, thông tin liên lạc của phụ huynh, hay bất cứ điều gì khác — những thông tin đó nằm riêng trên một bảng nội bộ chỉ thầy Nghĩa và các quản lý vùng mới xem được.

Nếu thấy dòng của mình có gì sai — tên viết sai, sai đội, sai level, hoặc sai khu vực — hãy báo cho quản lý vùng. Chỉ quản lý vùng mới sửa được — phụ huynh/học sinh không tự chỉnh được.
...
Tình trạng hiện tại của đội — cho mọi bài thi, mọi đội — nằm trên bảng công khai **Exams & Standing** (đường link ở trang [Organization](./organization.md)). Không cần mật khẩu để xem; ai cũng xem được bất cứ lúc nào. Tìm dòng của đội mình và cột `Status`, luôn hiển thị bằng câu chữ rõ nghĩa và có màu:
...
This section assumes you're already familiar with [Organization](./organization.md)'s regional-coordinator role and the Regional Pacing sheet where you set each team's cadence.
...
Family-visible comments from grading show up as a small note icon on
their own row's `Score` cell — hovering or clicking it shows exactly
what the grader wrote, without opening the (coordinator-only) grading
spreadsheet.
...
## 🇻🇳 Tiếng Việt — Dành cho Quản lý vùng

Phần này giả định đã quen với vai trò quản lý vùng ở trang [Organization](./organization.md) và bảng Regional Pacing nơi thiết lập nhịp độ của từng đội.
...
Nhận xét của người chấm mà gia đình được phép xem sẽ hiện dưới dạng một biểu tượng ghi chú nhỏ trên ô `Score` của đúng dòng đội đó — di chuột vào hoặc bấm vào sẽ thấy chính xác nhận xét của người chấm, mà không cần mở bảng chấm điểm (chỉ dành cho ban tổ chức).
...
English only (see this page's own top comment for why) -- this section is for Toan and Nghia, the two people who actually grade submissions each week. It's a distinct role from coordinating a region: grading is a judgment call on the math itself, not team pacing or being a family's first point of contact.
